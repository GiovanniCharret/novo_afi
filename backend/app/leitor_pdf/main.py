...
def new_concatenar_por_ponteiro_filtra_tabela_produtos(df, contexto):
    ...
    if todos_headers.empty:
        # Especialista puro — sem header detectável, devolve df vazio.
        # O orquestrador (concatenar_em_cascata) detecta via
        # _diagnosticar_tabela_produtos e tenta o próximo especialista.
        return pd.DataFrame(columns=['string_class', 'text'])
    ...
    if 'ncm' not in faixas or 'descricao' not in faixas:
        # Aliases não casam — este shape de NF não é coberto por esta função.
        # Devolve df vazio para o orquestrador tentar o próximo especialista.
        return pd.DataFrame(columns=['string_class', 'text'])
    ...
    if not linhas_ancora:
        # Sem NCM-âncora detectável — este shape não é coberto. Devolve df
        # vazio para o orquestrador tentar o próximo especialista.
        return pd.DataFrame(columns=['string_class', 'text'])
    ...
    for i, L in enumerate(linhas_ancora):
        ...
        # Função pura: NÃO levanta ValueError se um produto vier com ncm/descricao
        # vazios. O orquestrador detecta via _diagnosticar_tabela_produtos
        # (vê célula vazia ou contagem desbalanceada) e tenta o próximo especialista.
...
# =============================================================================
# Padrão Chain of Responsibility: cada função em ESPECIALISTAS_TABELA_PRODUTOS
# é especialista em UM shape estrutural de NF. Adicionar suporte a um shape
# novo = adicionar uma função à lista, sem refatorar as existentes.
# Vide docs/ORCHESTRATOR_PROD_TABLE_FUNCTIONS.md.

# Contadores globais — o laço per-NF é sequencial, sem necessidade de lock
# (threads ficam restritas ao cleaner LLM em description_cleaner.py).
_CASCADE_VITORIAS = Counter()   # {fn_name | 'humano': int}
_CASCADE_MOTIVOS = {}           # {fn_name: Counter({categoria: count})}


def _categoria_motivo(problema):
    """Resume a string-motivo em uma categoria curta para o relatório agregado.
    A string original continua disponível em log.json para análise caso a caso."""
    if problema.startswith('classes ausentes'):
        return 'classes ausentes'
    if 'célula(s) vazia(s)' in problema:
        return 'células vazias'
    if 'contagens desbalanceadas' in problema:
        return 'contagens desbalanceadas'
    if problema == 'df vazio':
        return 'df vazio'
    return 'exceção upstream'


def _diagnosticar_tabela_produtos(df):
    """Validação ESTRUTURAL da saída de um especialista. Retorna None se íntegra,
    ou string explicando o problema (para log e para o erro do orquestrador).

    Critério: tabela íntegra = todas as 5 classes canônicas presentes,
    contagens iguais entre elas, nenhuma célula vazia em texto.

    NÃO valida formato monetário — isso é trabalho de _tem_valor_numerico em
    consolidate_data_to_dict, downstream. Aqui é só 'tem forma de tabela?'.
    """
    if df is None or df.empty:
        return "df vazio"

    classes_obrigatorias = {'Descrição do produto', 'NCM/SH', 'QUANT', 'UNIT', 'price'}
    presentes = set(df['string_class'].dropna().unique())
    faltando = classes_obrigatorias - presentes
    if faltando:
        return f"classes ausentes: {sorted(faltando)}"

    # Cada classe deve aparecer o mesmo número de vezes (1 por produto).
    contagens = {c: int((df['string_class'] == c).sum()) for c in classes_obrigatorias}
    if len(set(contagens.values())) != 1:
        return f"contagens desbalanceadas: {contagens}"

    # Nenhuma célula das 5 canônicas pode ter texto vazio após strip.
    textos_strip = df['text'].astype(str).str.strip()
    canonica = df['string_class'].isin(classes_obrigatorias)
    vazios = df[canonica & (textos_strip == '')]
    if not vazios.empty:
        classes_com_vazio = vazios['string_class'].unique().tolist()
        return f"{len(vazios)} célula(s) vazia(s) em {classes_com_vazio}"
...
def concatenar_em_cascata(df, contexto):
    """
    Orquestrador da cascata para extrair a tabela de produtos.

    Filosofia (Chain of Responsibility):
        Cada especialista em ESPECIALISTAS_TABELA_PRODUTOS é PURO — produz
        dados, não decide destino. A regra 'broken → próximo' vive AQUI, no
        orquestrador, para que adicionar/remover especialistas não exija
        editar cada um internamente.

    Fluxo:
        1. Cada especialista recebe uma CÓPIA do df ORIGINAL — sem
           contaminação cruzada (A não pode poluir o que B vê).
        2. O resultado passa por _diagnosticar_tabela_produtos.
        3. Primeiro especialista com tabela íntegra vence.
        4. Se NENHUM produz íntegra, levanta ValueError — o CALLER decide o
           destino (retry alt-page para DANFE multi-folha, ou
           _montar_df_produtos_humano em última instância).

    Telemetria: incrementa _CASCADE_VITORIAS[fn_name] em vitória; acumula
    motivos categorizados em _CASCADE_MOTIVOS[fn_name] em falha. Relatório
    agregado emitido por relatar_cascade() ao fim do laço per-NF.
    """
    motivos = []
    for fn in ESPECIALISTAS_TABELA_PRODUTOS:
        nome = fn.__name__
        try:
            # df.copy() — isolamento. Cada especialista vê o ORIGINAL intacto.
            resultado = fn(df.copy(), contexto)
        except Exception as e:
            # Exceção inesperada (KeyError, IndexError, etc.) não bloqueia a
            # cascata — registra e segue para o próximo especialista.
            motivos.append(f"{nome}: {type(e).__name__}: {e}")
            _CASCADE_MOTIVOS.setdefault(nome, Counter())['exceção upstream'] += 1
            continue

        problema = _diagnosticar_tabela_produtos(resultado)
        if problema is None:
            # Vitória — contabiliza e devolve este resultado.
            _CASCADE_VITORIAS[nome] += 1
            return resultado

        # Tabela quebrada — registra motivo categorizado e tenta o próximo.
        motivos.append(f"{nome}: {problema}")
        _CASCADE_MOTIVOS.setdefault(nome, Counter())[_categoria_motivo(problema)] += 1

    # Esgotou a cascata. Caller decide o destino via except ValueError.
    raise ValueError(
        f"concatenar_em_cascata: nenhum especialista extraiu a tabela. "
        f"Motivos: {' | '.join(motivos)}. Contexto: {contexto}"
    )


def relatar_cascade():
    """Imprime resumo do batch + linha estruturada em log.json. Chamado UMA vez
    ao fim do laço per-NF (antes do cleaner LLM). Silencioso se nenhuma NF
    de produto foi processada."""
    total = sum(_CASCADE_VITORIAS.values())
    if total == 0:
        return  # nenhuma NF de produto no batch — nada a relatar

    linhas = [f"\n[Cascata Tabela Produtos]",
              f"  NFs de produto processadas       : {total}"]

    # Vitórias na ordem fixa: especialistas conforme a lista + 'humano' no fim.
    ordem = [fn.__name__ for fn in ESPECIALISTAS_TABELA_PRODUTOS] + ['humano']
    for nome in ordem:
        n = _CASCADE_VITORIAS.get(nome, 0)
        pct = (n / total * 100) if total else 0.0
        rotulo = '_montar_df_produtos_humano' if nome == 'humano' else nome
        linhas.append(f"  {rotulo:35s}: {n:4d} ({pct:5.1f}%)")

    # Motivos por especialista — categorias mais frequentes primeiro.
    for fn in ESPECIALISTAS_TABELA_PRODUTOS:
        nome = fn.__name__
        motivos_fn = _CASCADE_MOTIVOS.get(nome)
        if not motivos_fn:
            continue
        n_falhas = sum(motivos_fn.values())
        linhas.append(f"\n  Motivos pelos quais {nome} não atendeu (n={n_falhas}):")
        for categoria, count in motivos_fn.most_common():
            linhas.append(f"    {categoria:33s}: {count}")

    print('\n'.join(linhas) + '\n')

    # Linha estruturada em log.json — payload completo para auditoria entre runs.
    log = {
        'tipo': 'resumo_cascade_produtos',
        'total': total,
        'vitorias': dict(_CASCADE_VITORIAS),
        'motivos': {nome: dict(motivos) for nome, motivos in _CASCADE_MOTIVOS.items()},
    }
    try:
        with open("log.json", "a", encoding="utf-8") as f:
            f.write(json.dumps(log, ensure_ascii=False) + "\n")
    except OSError as e:
        print(f"  [aviso] não foi possível anexar resumo da cascata ao log.json: {e}")
...
def _montar_df_produtos_humano(nome_saida, contexto_erro=""):
    ...
    É o ÚNICO ponto de fallback humano para a tabela de produtos — chamado
    pela cascata (após esgotamento de especialistas) e pelo retry alt-page.
    Por isso, incrementa _CASCADE_VITORIAS['humano'] aqui: ponto único = conta
    única, independente de qual rota disparou.
    """
    _CASCADE_VITORIAS['humano'] += 1
...
def extrair_produtos_pagina_alternativa(pdf_path, page_index, nome_saida):
    ...
    try:
        ...
        # Cascata de especialistas. Se nenhum produzir tabela íntegra,
        # concatenar_em_cascata levanta ValueError, capturado pelo except
        # abaixo (linha ~1242), que delega para _montar_df_produtos_humano.
        return concatenar_em_cascata(tabela_filtrada, nome_saida)
...
def consolidate_data_to_dict(list_product_transation, *args):
    ...
    CAMPOS_NUMERICOS = ('valor', 'preco_unitario')

    def _tem_valor_numerico(v):
        """True se v carrega conteúdo numérico. Rejeita rótulos sem dígito
        (ex.: 'Base Cálculo:'). Propositalmente leniente: '.' e ',' aparecem
        de formas variadas nas extrações do pdfplumber/OCR — validar formato
        de moeda aqui rejeitaria valores legítimos ('160', '160. 569,92')."""
    ...
    def _campo_invalido(key, v):
        """Campo precisa de revisão humana: None, string vazia, ou — para
        valor/preco_unitario — string sem nenhum dígito."""
        if v is None:
            return True
        if isinstance(v, str) and not v.strip():
            return True
        if key in CAMPOS_NUMERICOS and not _tem_valor_numerico(v):
            return True
        return False
    ...
    for trans in list_product_transation:
        ...
        for key in transacao.keys():
            ...
            if _campo_invalido(key, transacao[key]):

                for arg in args:     
           
                    if key in arg and not _campo_invalido(key, arg[key]):
                        transacao[key] = arg[key]
                        break
                    

        # 3. Validação de preenchimento — None, string vazia, ou (valor/
        # preco_unitario) string que não parece moeda → revisão humana.
        campos_vazios = [k for k, v in transacao.items() if _campo_invalido(k, v)]
...
def validar_total_contra_contrato(df, contrato):
    ...
    def _parse_brl(v):
        ...
        if isinstance(v, (int, float)):
            return float(v)
        # ''.join(split()) remove TODO espaço — inclusive interno, artefato
        # comum de OCR e de digitação humana (ex.: '160 569,92').
        s = ''.join(str(v).split()).replace('R$', '')
        s = s.replace('.', '').replace(',', '.')
        return s

    serie_normalizada = df['valor'].map(_parse_brl)
    # errors='raise': um valor não-numérico aqui significa que escapou de TODAS
    # as validações de consolidate_data_to_dict — é bug de código (tipo 2), não
    # algo que o operador resolva no terminal. O batch PARA e o parser é corrigido.
    serie_float = pd.to_numeric(serie_normalizada, errors='raise')
    if serie_float.isna().any() or (serie_float <= 0).any():
        raise RuntimeError(
            "validar_total_contra_contrato: valor de NF ausente ou <= 0 no "
            "consolidado. Falha de extração que escapou da revisão humana — "
            "corrija o parser."
        )
    soma = float(serie_float.sum())
...
for seq, arquivo in enumerate(tqdm(arquivos_pdf)):
    ...
    if not confirma_tipo_documento(df_nota):
        ...
        if invoice_type == 'product':
            ...
            if not df_product_service_desciption['tabela_produtos'].empty:
                try:
                    ...
                    # Cascata de especialistas — sem flag, sem branch binário.
                    # Se nenhum entregar tabela íntegra, concatenar_em_cascata
                    # levanta ValueError; o except abaixo decide o destino
                    # (alt-page retry para multi-folha, ou _montar_df_produtos_humano).
                    df_product_dict = concatenar_em_cascata(product_sheet_analysed, nome_saida)
            ...
            if list_product_service_transation is None:
                if eh_danfe_multifolha(arquivo):
                    ...
                    # Cascata esgotada e NÃO é multi-folha (não há retry alt-page).
                    # Fallback humano via _montar_df_produtos_humano — ponto único
                    # de revisão humana para tabela de produtos. Contabilizado em
                    # _CASCADE_VITORIAS['humano']. Garante que o script nunca para
                    # por falha de extração.
                    df_product_dict_humano = _montar_df_produtos_humano(
                        nome_saida, contexto_erro=str(erro_pipeline)
                    )
                    list_product_service_transation = get_real_transations(df_product_dict_humano)
                else:
                    raise ValueError(
        ...
        # consolidate_data_to_dict valida cada campo (None / string vazia /
        # valor não-monetário). Em caso de falha, levanta ValueError listando
        # os campos; pedimos ao operador e repetimos — laço até a NF ficar
        # íntegra (princípio: NF não passa inválida).
        ...
        while True:
            try:
                nf_data_tabulated = consolidate_data_to_dict(
                    ...
                    data_nota_fiscal, nome_fornecedor, cnpj_fornecedor, CONTRATO,
                    preenchimentos_extras
                )
                break
            except ValueError as e:
                # Extrai os campos sinalizados do texto da exceção e pede ao operador.
...
# Resumo da cascata de especialistas — vitórias e motivos no terminal + log.json.
relatar_cascade()


"""
5 - Limpeza semântica da coluna descricao via LLM (batch único pós-laço)C
"""
