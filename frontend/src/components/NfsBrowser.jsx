...
/**
 * F3b — Consulta de NFs do contrato ativo na sessão.
 *
 * O contrato exibido é o `selectedContrato` da sessão (definido em
 * "Trocar contrato" / aba "Contratos"). Não há dropdown local — a
 * refatoração de 2026-05-27 removeu o seletor que duplicava a função
 * da página de troca e confundia o fluxo.
 *
 * Filtros locais (q, datas, valores, tipo_nota) persistem quando o
 * contrato ativo muda — uma busca em andamento não é desfeita por
...
export default function NfsBrowser({ selectedContrato }) {
  ...
  return (
    <section className="card nfs-card">
      <div className="card-header">
        <div>
          <p className="section-kicker">Visualização e download</p>
          <h2 className="card-title">Notas fiscais do contrato</h2>
        </div>
      </div>
      ...
      <div className="nfs-filter-bar">
        ...
        <button
          ...
          onClick={() => exportNfsResumo(nfs, selectedContrato?.numero)}
        >
          Exportar Excel
        </button>
      </div>
      ...
      {selectedContrato && <TotalizadoresCard contrato={selectedContrato} />}
